...
class CompositeRootkit(interfaces.plugins.PluginInterface):
    ...
    def _subsystem_b3(
        ...
        try:
            ...
            vollog.debug("B3 skipped: proc_root dereference failed")
            return evidence
        ...
        for field in ops_fields:
            try:
                ...
                if candidate is None:
                    continue
                ops_obj = candidate.dereference() if hasattr(candidate, "dereference") else candidate
                ops_name = field
                break
            except Exception:
                continue
        ...
        for candidate_field in ("seq_ops", "proc_ops", "ops"):
            try:
                ...
                if candidate is not None:
                    seq_obj = candidate.dereference() if hasattr(candidate, "dereference") else candidate
                    seq_source = f"proc_root.{candidate_field}"
                    break
            except Exception:
                continue
        if seq_obj is None and hasattr(ops_obj, "seq_ops"):
            try:
    ...
    def _b3_inspect_ops_fields(
        ...
        for field in field_names:
            if not hasattr(ops_obj, field):
                continue
            try:
                raw_ptr = int(getattr(ops_obj, field) or 0)
            except Exception:
                continue
