...
class CompositeRootkit(interfaces.plugins.PluginInterface):
    ...
    def _c1_collect_pid_view(self) -> Dict[int, Tuple[int, str]]:
        ...
        try:
            ...
            upid_chain_offset = self._get_offset(self.vmlinux, "upid", "pid_chain")
            pid_numbers_offset = self._get_offset(self.vmlinux, "pid", "numbers")
        except Exception:
        ...
        try:
            for bucket in pidhash:
                try:
                    ...
                    while node:
                        try:
                            upid_addr = int(node.vol.offset) - int(upid_chain_offset)
                            pid_addr = upid_addr - int(pid_numbers_offset)
                            pid_obj = self.vmlinux.object(
                                object_type="pid",
                                offset=pid_addr,
                                absolute=True,
                            )
                            ...
                            if task_node:
                                ...
                                vollog.debug(
                                    "C1 node_offset=0x%x, upid_addr=0x%x, pid_addr=0x%x, task_addr=0x%x",
                                    int(node.vol.offset),
                                    upid_addr,
                                    pid_addr,
                                    task_addr,
