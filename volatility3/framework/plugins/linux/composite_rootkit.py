...
class SubsystemA:
    ...
    def run(self) -> SubsystemAResult:
        ...
        if "module_kset" not in missing:
            self.plugin.invariant(len(K) > 0, "K must not be empty when module_kset exists")
        if "vmap_area_list" not in missing:
            self.plugin.invariant(len(V) > 0, "V must be non-trivial when vmap_area_list exists")
        ...
        filtered = {
            ...
            if not any(self.plugin.overlap(region.base, region.end, f.base, f.end) for f in filters)
        }

        for region in filtered:
            self.plugin.invariant(self.plugin.page_aligned(region.base), "AnomalousRegions base must be page-aligned")
            self.plugin.invariant(self.plugin.is_canonical(region.base), "AnomalousRegions base must be canonical")
    ...
    def _collect_modules_raw(self) -> Set[int]:
        raw: Set[int] = set()
        for module in lsmod.Lsmod.list_modules(self.plugin.context, self.plugin.config["kernel"]):
    ...
    def _collect_module_kset_raw(self) -> Tuple[Set[int], Set[str]]:
        ...
        missing: Set[str] = set()
        try:
            module_kset = self.plugin.vmlinux.object_from_symbol(symbol_name="module_kset")
    ...
    def _collect_vmap_raw(self) -> Tuple[Set[int], Set[str]]:
        ...
        missing: Set[str] = set()
        try:
            vmap_area_list = self.plugin.vmlinux.object_from_symbol(symbol_name="vmap_area_list")
    ...
    def _page_aligned_carving(self) -> Set[Tuple[int, int, str]]:
        ...
        for A in range(start, end, 0x1000):
            ...
            try:
                mod = self.plugin.vmlinux.object(object_type="module", offset=A, absolute=True)
            except Exception:
                continue
            ...
            try:
                ...
                self.plugin.invariant(self.plugin.is_canonical(base), "carving base must be canonical")
                self.plugin.invariant(size < 100 * 1024 * 1024, "carving size must be <100MB")
                self.plugin.invariant(self.plugin.page_aligned(base), "carving base must be page-aligned")
    ...
    def _collect_bpf_jit_regions(self) -> Set[Region]:
        ...
        try:
            ...
            for bucket in prog_idr.idr_rt.to_list(self.plugin.vmlinux.symbol_table_name + "!radix_tree_node", "parent"):
                try:
                    prog = self.plugin.vmlinux.object(object_type="bpf_prog", offset=int(bucket.vol.offset), absolute=True)
                    jited = int(getattr(prog, "bpf_func", 0) or 0)
                    jited_len = int(getattr(prog, "jited_len", 0) or 0)
                    if jited and jited_len:
                        regions.add(Region(self.plugin.canonicalize(jited), jited_len, "filter", "eBPF-JIT"))
                except Exception:
                    continue
        except Exception as exc:
    ...
    def _collect_ftrace_tramp_regions(self) -> Set[Region]:
        regions: Set[Region] = set()
        try:
            ftrace_ops_list = self.plugin.vmlinux.object_from_symbol(symbol_name="ftrace_ops_list")
            list_off = self.plugin.get_offset("ftrace_ops", "list")
            for node in ftrace_ops_list.to_list(self.plugin.vmlinux.symbol_table_name + "!list_head", "next"):
                try:
                    ops = self.plugin.vmlinux.object(
                        object_type="ftrace_ops", offset=int(node.vol.offset) - list_off, absolute=True
                    )
                    callback = int(getattr(ops, "func", 0) or 0)
                    if callback:
                        regions.add(Region(self.plugin.canonicalize(callback), 0x1000, "filter", "ftrace-callback"))
                except Exception:
                    continue
        except Exception as exc:
    ...
    def _collect_kprobe_tramp_regions(self) -> Set[Region]:
        regions: Set[Region] = set()
        try:
            kprobe_table = self.plugin.vmlinux.object_from_symbol(symbol_name="kprobe_table")
            hlist_off = self.plugin.get_offset("kprobe", "hlist")
            for bucket in kprobe_table:
                for node in bucket.to_list(self.plugin.vmlinux.symbol_table_name + "!hlist_node", "next"):
                    try:
                        kp = self.plugin.vmlinux.object(
                            object_type="kprobe", offset=int(node.vol.offset) - hlist_off, absolute=True
                        )
                        for field in ("pre_handler", "post_handler", "fault_handler", "break_handler"):
                            ptr = int(getattr(kp, field, 0) or 0)
                            if ptr:
                                regions.add(Region(self.plugin.canonicalize(ptr), 0x1000, "filter", f"kprobe-{field}"))
                    except Exception:
                        continue
        except Exception as exc:
    ...
    def _assert_filter_safety(self, filters: Set[Region]) -> None:
        ...
        for region in filters:
            overlaps_legit = any(self.plugin.overlap(region.base, region.end, s, e) for s, e in legit)
...
class SubsystemB:
    ...
    def run(self) -> SubsystemBResult:
        ...
        self.plugin.invariant(len(whitelist) > 0, "Whitelist must contain at least kernel text")
    ...
    def _check_syscalls(self, whitelist: Sequence[Tuple[int, int]]) -> Set[HookRecord]:
        ...
        try:
            ...
            for idx, ptr_obj in enumerate(table):
                ...
                self.plugin.invariant(self.plugin.is_canonical(p), "syscall pointer must be canonical")
    ...
    def _check_idt(self, whitelist: Sequence[Tuple[int, int]]) -> Tuple[Set[HookRecord], bool]:
        findings: Set[HookRecord] = set()
        flagged = 0
        total = 0
        try:
            ...
            for idx, gate in enumerate(idt):
                ...
                if p == 0:
                    continue
                self.plugin.invariant(self.plugin.is_canonical(p), "IDT pointer must be canonical")
                if self.plugin.in_whitelist(p, whitelist) or self.plugin.in_entry_regions(p):
                    continue
                flagged += 1
        ...
        if total > 0 and flagged > total // 2:
            vollog.error("SubsystemB IDT invalid detection: %d/%d entries flagged", flagged, total)
            return set(), True
    ...
    def _check_vfs(self, whitelist: Sequence[Tuple[int, int]]) -> Set[HookRecord]:
        findings: Set[HookRecord] = set()
        try:
            proc_root_sym = self.plugin.vmlinux.object_from_symbol(symbol_name="proc_root")
            proc_root = proc_root_sym.dereference() if hasattr(proc_root_sym, "dereference") else proc_root_sym
            fops = getattr(proc_root, "proc_dir_operations", getattr(proc_root, "proc_fops", None))
            if fops is None:
                return findings
            fops = fops.dereference() if hasattr(fops, "dereference") else fops
            for name in ("open", "read", "write", "iterate", "iterate_shared", "readdir"):
        ...
        try:
            ...
            for proto_idx, hook_arr in enumerate(nf_hooks):
                for hook_idx, list_head in enumerate(hook_arr):
                    for hook in list_head.to_list(self.plugin.vmlinux.symbol_table_name + "!nf_hook_ops", "list"):
                        p = self.plugin.canonicalize(int(getattr(hook, "hook", 0) or 0))
                        if p and not self.plugin.in_whitelist(p, whitelist):
                            findings.add(HookRecord("netfilter", p, f"nf_hooks[{proto_idx}][{hook_idx}]"))
        except Exception as exc:
            ...
            for bidx, bucket in enumerate(kprobe_table):
                for node in bucket.to_list(self.plugin.vmlinux.symbol_table_name + "!hlist_node", "next"):
                    ...
                    for field in ("pre_handler", "post_handler", "fault_handler", "break_handler"):
                        ...
                        if p and not self.plugin.in_whitelist(p, whitelist):
                            findings.add(HookRecord("kprobe", p, f"kprobe[{bidx}].{field}"))
        except Exception as exc:
...
class SubsystemC:
    ...
    def run(self) -> SubsystemCResult:
        ...
        for task_addr in hidden_ptrs:
            ...
            if not self.plugin.valid_ascii_name(comm, min_len=1, max_len=16):
                continue
            hidden.add(HiddenTask(pid=pid, task_addr=task_addr, comm=comm, source="runq/pidhash"))
    ...
    def _collect_task_set_from_list(self) -> Set[int]:
        out: Set[int] = set()
        try:
            for task in pslist.PsList.list_tasks(self.plugin.context, self.plugin.config["kernel"]):
    ...
    def _collect_task_set_from_runqueues(self) -> Set[int]:
        ...
        try:
            ...
            nr_cpu_ids = int(self.plugin.vmlinux.object_from_symbol(symbol_name="nr_cpu_ids"))
    ...
    def _collect_task_set_from_pid_hash(self) -> Set[int]:
        ...
        try:
            ...
            for bucket in pidhash:
                for upid in bucket.to_list(self.plugin.vmlinux.symbol_table_name + "!upid", "pid_chain"):
                    try:
                        pid_obj = upid.pid
    ...
    def _safe_task(self, task_addr: int):
        try:
            return self.plugin.vmlinux.object(object_type="task_struct", offset=task_addr, absolute=True)
        except Exception:
            return None
...
class CompositeRootkit(interfaces.plugins.PluginInterface):
    ...
    def _generator(self) -> Iterator[Tuple[int, Tuple[object, ...]]]:
        ...
        if not any([a_result.anomalous_regions, b_result.hooks, c_result.hidden_tasks, agg.correlations]):
            yield (0, (agg.detection_type, "Info", "NoFindings", format_hints.Hex(0), "No anomalies found"))
    ...
    def normalize_ptr(self, kind: str, ptr: int) -> int:
        ...
        if kind == "kobj":
            ...
            try:
                va = self.vmlinux.object(object_type="vmap_area", offset=p, absolute=True)
                p = self.canonicalize(int(getattr(va, "va_start", getattr(va, "addr", p))))
            except Exception:
                p = self.canonicalize(p)

        self.invariant(self.is_canonical(p), "f(x) must produce canonical kernel pointer")
    ...
    def get_kaslr_shift(self) -> int:
        try:
            sym = self.context.symbol_space.get_symbol(self.vmlinux.symbol_table_name + "!init_task").address
            runtime = int(self.vmlinux.object_from_symbol(symbol_name="init_task").vol.offset)
            return runtime - sym
        except Exception:
            return 0
    ...
    def get_modules_range(self) -> Tuple[int, int]:
        try:
            start = int(self.vmlinux.object_from_symbol(symbol_name="module_alloc_base"))
        ...
        return min(x[0] for x in ranges) - 0x8000000, max(x[1] for x in ranges) + 0x8000000
    ...
    def build_whitelist(self) -> List[Tuple[int, int]]:
        ...
        def sym_range(start_sym: str, end_sym: str) -> Optional[Tuple[int, int]]:
            try:
                s = self.context.symbol_space.get_symbol(self.vmlinux.symbol_table_name + "!" + start_sym).address + kaslr
                e = self.context.symbol_space.get_symbol(self.vmlinux.symbol_table_name + "!" + end_sym).address + kaslr
        ...
        for sym in ("__entry_text_start", "__entry_text_end", "entry_trampoline", "cpu_entry_area"):
            try:
    ...
    def in_entry_regions(self, ptr: int) -> bool:
        ...
        for sym in ("__entry_text_start", "entry_trampoline", "cpu_entry_area"):
            try:
                base = self.canonicalize(int(self.vmlinux.object_from_symbol(symbol_name=sym).vol.offset))
