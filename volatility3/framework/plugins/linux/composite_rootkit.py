...
class CompositeRootkit(interfaces.plugins.PluginInterface):
    ...
    def _find_list_poison_pages(self, start: int, end: int) -> Set[int]:
        ...
        while cur < end:
            ...
            while pos != -1:
                ...
                vollog.debug("module_base: 0x%x", module_base)
    ...
    def _reconstruct_module(
        ...
        if not self._valid_ascii_name(name):
            vollog.debug("reconstruction: failure (invalid name) at 0x%x", page_addr)
            return None
    ...
    def _locate_sys_call_table(self, kernel_start: int, kernel_end: int) -> Optional[int]:
        ...
        for addr in range(kernel_start, scan_end, PAGE_SIZE):
            try:
                ...
                continue
            good = 0
            first_good_offset: Optional[int] = None
            ...
            if good >= 128:
                if first_good_offset is None:
                    first_good_offset = 0
                table = addr + first_good_offset
                vollog.debug("sys_call_table located by heuristic")
                vollog.debug("candidate_page=0x%x", addr)
                vollog.debug("first_good_offset=0x%x", first_good_offset)
                vollog.debug("final_sys_call_table=0x%x", table)
                return table
        return None
